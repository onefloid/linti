...
function applyRuleQuery(id: unknown) {
...
onMounted(() => applyRuleQuery(route.query.rule))
...
watch([code, procedure, selectedId], () => {
  if (pending) finish()
})
...
function run(fix = false) {
  ...
  getWorker().postMessage({ id: pending.id, code: code.value, procedure: procedure.value, ruleId: selected.value.id, fix })
}
